...
// Made by FlorianMgs: https://github.com/FlorianMgs
...
contract MultiLottery {
    ...
    function getTotalPrize() public view returns (uint) {
        return _getRewardAmount(address(this).balance);
    }
    ...
    function getCurrentDraw(uint _lotteryId) public view returns (uint) {
        return lotteriesMapping[_lotteryId].drawId;
    }
    ...
    // Setters
    function setLotteryEntryPriceMinMax(uint _lotteryId, uint _lotteryPriceMaxInWei, uint _lotteryPriceMinInWei) public onlyOwner {
    ...
    function enter(uint _lotteryId) public payable {
        ...
        emit NewPlayer(_lotteryId, lotteriesMapping[_lotteryId].players.length, msg.value, getLotteryPrize(_lotteryId), getTotalPrize());
    }
    ...
    function getRandomNumber(uint _ownerSalt) public view returns (uint) {
        return uint(
            keccak256(
                abi.encodePacked(
                    _ownerSalt,
    ...
    function pickWinner(uint _lotteryId, uint _ownerSalt) public onlyOwner {

        // determine winner
        uint index = getRandomNumber(_ownerSalt) % lotteriesMapping[_lotteryId]._playersPool.length;
        ...
        emit NewWinner(_lotteryId, lotteriesMapping[_lotteryId].drawId, lotteriesMapping[_lotteryId]._playersPool[index], _rewardAmount, getTotalPrize());
